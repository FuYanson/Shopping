									...
									<div class="box-inner order-cart order-list-cart clear">
										...
										<div class="gray-sub-title cart-title">
											...
											<span class="order-id"> 订单号： 
                                                <a href="javascript:;">{{receiveInfo_landLine[0].landLine}}</a> </span>
											<span class="order-detail">
                                                <a href="javascript:;" @click="$router.push('/MyShoppingCart/'+receiveInfo_landLine[0].landLine)">查看详情&gt;</a>     
                                            </span> 
                                        ...
                                        <div class="prod-operation">
											<div class="total">¥ {{this.$store.getters.My_price}}</div>
											<div class="status">
                                                <a class="blue-small-btn js-payment-order" >现在付款</a>
                                            </div>
										</div>
                                        <!-- 商品栏 -->
										<div class="cart" v-for="item in cartList" :key="item.sku_id">
											<div class="cart-items clear">
                                            <div class="prod-info clear">
                                            <div class="items-thumb">
                                                <a href="javascript:;" target="_blank">
                                                    <img :src="item.ali_image"></a>
                                            </div>
...
export default {
  ...
  created() {
    console.log(addList.receiveInfo);
  },
  computed: {
    cartList() {
    ...
    receiveInfo_landLine() {
      return addList.receiveInfo.filter(item => item.default);
    }
  }
};
</script>
